...
//       </div>
//       <div className="w-full grid  [grid-template-columns:repeat(auto-fill,minmax(200px,1fr))] gap-4 gap-y-6 px-3 sm:px-0">
...
// 1. هنجيب Link ونشيل useNavigate (مش محتاجينه هنا)
import { useContext, useState, useEffect } from "react"; 
...
const specialities = [     
  ...
  return (     
    <div>       
      <h2 className="text-lg font-medium p-2">Browse through the doctors specialist.</h2>             
      <div className="flex flex-col sm:flex-row items-start gap-5 mt-5">                  
        <div className="flex flex-col gap-4 text-sm text-gray-600 ">           
          {specialities.map(spec => (             
            <Link               
              key={spec}               
              to={speciality === spec ? '/doctors' : `/doctors/${spec}`}               
              className={`w-[94vw] sm:w-auto pl-3 py-1.5 pr-16 border border-gray-300 rounded transition-all                           
                          ${speciality === spec ? "bg-indigo-100 text-black" : "hover:bg-gray-100"}                          
